...
class FileComparator:
    ...
    def refresh_list(self):
        ...
        if self.file_listbox.size() > 0:
            if current_selection >= self.file_listbox.size():
                current_selection = self.file_listbox.size() - 1
            self.file_listbox.selection_set(current_selection)
            self.current_index = current_selection
    ...
    def show_comparison(self):
        ...
        self.create_comparison_title(name)
        self.create_previews_frame(files, thumbnail_size)
        self.create_actions_frame(files, thumbnail_size, name)

    def create_comparison_title(self, name):
        title_label = tk.Label(self.canvas_frame, text=f"Comparing: {name}", font=("Arial", 14, "bold"))
        title_label.pack(pady=(0, 10))

    def create_previews_frame(self, files, thumbnail_size):
        previews_frame = tk.Frame(self.canvas_frame, height=int(0.2 * self.canvas_frame.winfo_toplevel().winfo_height()))
        previews_frame.pack(fill=tk.X, expand=False)

        paths_label = tk.Label(previews_frame, text="Paths of compared files:", font=("Arial", 12))
        paths_label.pack(fill=tk.X, pady=5)

        for i, (size, path) in enumerate(files):
            frame = tk.Frame(previews_frame)
            frame.pack(side=tk.LEFT, padx=10, pady=10)
            create_path_frame(frame, thumbnail_size, i, size, path)

    def create_actions_frame(self, files, thumbnail_size, name):
        actions_frame = tk.Frame(self.canvas_frame)
        ...
        for i, (size, path) in enumerate(files):
            ...
            self.create_delete_button(frame, path, name)
    ...
    def create_delete_button(self, frame, path, name):
        # Tworzy przycisk usuwania pliku
...
def create_action_buttons(frame, path):
    # Tworzy ramkę dla przycisków akcji
    buttons_frame = tk.Frame(frame)
    buttons_frame.pack(fill=tk.X, pady=5)

    # Przycisk otwierający lokalizację pliku
...
def display_file(file_path, frame, thumbnail_size):
    # Tworzy ramkę do wyświetlania podglądu pliku o określonym rozmiarze miniatury
    preview_frame = tk.Frame(frame, width=thumbnail_size, height=thumbnail_size)
    preview_frame.pack(pady=5)
    
    # Sprawdza, czy plik jest obrazem (PNG, JPG, JPEG)
    if file_path.lower().endswith(('.png', '.jpg', '.jpeg')):
        try:
            # Otwiera obraz, tworzy miniaturę i wyświetla ją w etykiecie
            ...
            # Wyświetla komunikat o błędzie, jeśli nie uda się załadować obrazu
            error_label = tk.Label(preview_frame, text="Error loading image")
            error_label.pack()
    
    # Sprawdza, czy plik jest wideo (MP4)
    elif file_path.lower().endswith('.mp4'):
        try:
            # Otwiera wideo, odczytuje pierwszą klatkę, tworzy miniaturę i wyświetla ją w etykiecie
            ...
            # Wyświetla komunikat o błędzie, jeśli nie uda się załadować wideo
...
def create_delete_button(frame, path, name):
    # Tworzy przycisk usuwania pliku
    delete_button = tk.Button(frame, text="Delete", bg="#ff9999", 
                              command=lambda p=path, f=frame, n=name: app.delete_file(p, f, n))
    delete_button.pack(fill=tk.X, pady=5)

def format_size(size):
    # Konwertuj rozmiar na KB i sformatuj z odstępami co trzy cyfry
    size_kb = size / 1024
    return f"{size_kb:,.0f} KB"

def create_path_frame(frame, thumbnail_size, i, size, path):
    # Utwórz ramkę z zawiniętym tekstem dla ścieżki
    path_frame = tk.Frame(frame, width=thumbnail_size)  # Tworzy ramkę o szerokości równej thumbnail_size
    path_frame.pack(fill=tk.X)  # Ustawia ramkę, aby wypełniała przestrzeń w poziomie
    # path_frame.pack_propagate(False)  # Nie pozwala ramce zmieniać rozmiaru, aby dopasować się do zawartości
    
    # Sformatuj rozmiar pliku
    formatted_size = format_size(size)
    
    # Etykieta z informacjami o pliku z powiększoną czcionką
    info_label = tk.Label(path_frame, 
                          text=f"Found_{i+1}\nSize: {formatted_size}",  # Tekst etykiety zawiera numer znalezionego pliku i jego rozmiar
                          wraplength=thumbnail_size)  # Ustawia długość zawijania tekstu na thumbnail_size
    info_label.pack(fill=tk.X)  # Ustawia etykietę, aby wypełniała przestrzeń w poziomie
    
    # Etykieta ścieżki z zawijaniem tekstu
    path_label = tk.Label(path_frame, 
                          text=f"Path: {path}",  # Tekst etykiety zawiera ścieżkę do pliku
                          wraplength=thumbnail_size,  # Ustawia długość zawijania tekstu na thumbnail_size
                          justify=tk.LEFT)  # Ustawia wyrównanie tekstu do lewej
    path_label.pack(fill=tk.X)  # Ustawia etykietę, aby wypełniała przestrzeń w poziomie
