...
class FileComparator:
    def __init__(self, root):
        ...
        self.file_list_frame.pack(side=tk.RIGHT, fill=tk.Y)
        
        self.file_listbox = tk.Listbox(self.file_list_frame, height=20)
        self.file_listbox.pack(side=tk.LEFT, fill=tk.Y)
        self.file_listbox.bind("<<ListboxSelect>>", self.select_file)
        
        self.refresh_button = tk.Button(self.file_list_frame, text="Refresh", command=self.refresh_list)
    ...
    def show_comparison(self):
        ...
        for i, (size, path) in enumerate(files):
            ...
            delete_button = tk.Button(frame, text="Delete", command=lambda p=path, f=frame: self.delete_file(p, f))
            delete_button.pack()
    ...
    def delete_file(self, file_path, frame):
        try:
            os.remove(file_path)
            frame.destroy()
            self.info_label.config(text=f"Deleted: {file_path}", fg="green")
            self.root.after(500, self.show_comparison)
            self.refresh_list()
        except Exception as e:
            self.info_label.config(text=f"Failed to delete: {file_path}", fg="red")
